 ...
 * - 브라우저 CORS 회피: frontend는 /api/acled/events 호출 → backend(5001)가 acleddata.com 호출
 * - 토큰은 backend/.env의 ACLED_API_TOKEN에만 두세요.
 */

const API_BASE = "";

export interface FetchACLEDOptions {
  country?: string;
  year?: number;
  limit?: number;
}

export interface FetchACLEDResult {
  events: OsintEvent[];
  count: number;
  error?: string | null;
}
...
export async function fetchACLEDEvents(
  options: FetchACLEDOptions = {}
): Promise<FetchACLEDResult> {
